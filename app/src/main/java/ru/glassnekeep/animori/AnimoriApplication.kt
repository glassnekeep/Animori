...
import ru.glassnekeep.character_data.di.DaggerCharacterDataComponent
...
class AnimoriApplication : Application() {
    ...
    override fun onCreate() {
        super.onCreate()
        val anilistDataProvider = DaggerAnilistComponent.factory().create()
        appProvider = DaggerAppComponent.factory().create(
            ...
            homeDataProvider = DaggerHomeComponent.factory().create(
                mediaDataProvider = DaggerMediaDataComponent.factory().create(parent = anilistDataProvider),
            ),
            characterDataProvider = DaggerCharacterDataComponent.factory().create(anilistDataProvider)
        )
    }
}
